...
function canPass(role)
		if gRoleNum[role] ~= 0 and not world:isPassLineBlocked(gRoleNum[role])  then
			return true
		else
			return false
		end
...
function canShoot(role)
		if gRoleNum[role] ~= 0 and  not world:isShootLineBlocked(gRoleNum[role]) then
			return true
		else
			return false
		end
